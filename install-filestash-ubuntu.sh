...
sudo apt-get install -y libglib2.0-0 curl gnupg > /dev/null
cat <<EOF | gpg --import
...
sudo apt install axel wget curl -y 
...
curl -L https://raw.githubusercontent.com/sehnryr/filestash/master/filestash.service -o filestash.service
sudo mv filestash.service /app/filestash.service
sudo systemctl enable /app/filestash.service

sudo useradd filestash
sudo chown -R filestash:filestash /app/
